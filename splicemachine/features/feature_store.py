...
class FeatureStore:
    ...
    def create_feature(self, *, feature_set_schema: str, feature_set_table: str,
        ...
        :return:
        """
        fset: FeatureSet = self.get_feature_sets(_filter={'table_name':feature_set_table, 'schema_name':feature_set_schema})[0]
    ...
    def run_feature_elimination(self, df, features, label: str = 'label', n: int = 10, verbose: int = 0,
        ...
        rnd = 0
        mlflow_results = []
        assert len(remaining_features) > n, "You've set the number of desired features (n) greater than the number of available features"
